...
import { BookOpen, Moon, Sun, Focus, Home } from 'lucide-react';
import { useTheme } from '../contexts/ThemeContext';
import { useApp } from '../contexts/AppContext';

const Header: React.FC = () => {
  const { isDark, toggleTheme } = useTheme();
  const { isFocusMode, setIsFocusMode, currentView, setCurrentView } = useApp();

  if (isFocusMode) return null;

  const navItems = [
    { key: 'landing', label: 'Trang chủ' },
    { key: 'tools', label: 'Công cụ học tập' },
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex items-center space-x-4 animate-fade-in-right">
            <button
              ...
              className="p-2 rounded-xl bg-purple-100 text-purple-600 hover:bg-purple-200 dark:bg-purple-900/30 dark:text-purple-400 dark:hover:bg-purple-900/50 transition-all duration-300 hover:scale-110 animate-pulse"
